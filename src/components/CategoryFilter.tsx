...
export function CategoryFilter({ selectedCategories, onChange }: CategoryFilterProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-900/40 border border-white/[0.06] rounded-3xl p-4 backdrop-blur-md">
      ...
      {selectedCategories.length > 0 && (
        <button
          onClick={handleClear}
          className="text-xs font-semibold text-blue-400 hover:text-blue-300 transition-colors self-end sm:self-auto cursor-pointer"
        >
          Reset filters
        </button>
      )}
